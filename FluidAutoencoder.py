...
class FluidAutoencoder:

    def __init__(self, topology=1, latent=8, plot=True):
        ...
        self.checkpoint_path = "cp_fluid/ae_latent" +str(self.latent_dimension) +"_topology" +str(self.id_topology) +".ckpt"
        self.opt = tf.keras.optimizers.Adam(lr=1e-5, decay=1e-5)
        self.plot = plot
        ...
        if self.id_topology == 1:
            ...
            self.encoder.add(Dense(self.latent_dimension, activation='relu'))           # 8192 -> latent

            # Setting Decoder architecture
            self.decoder.add(Dense(8*8*128, activation='relu'))                         # latent -> 8192
        ...
        if self.id_topology == 3:
            ...
            self.encoder.add(Dense(self.latent_dimension, activation='relu'))           # 4096 -> latent

            # Setting Decoder architecture
            self.decoder.add(Dense(8*8*64, activation='relu'))                          # latent -> 4096
        ...
        if self.id_topology == 4:
            ...
            # Setting Decoder architecture
            self.decoder.add(Dense(16*16*32, activation='relu'))                        # latent -> 8192
        ...
        if self.id_topology == 5:
            ...
            self.encoder.add(Dense(self.latent_dimension, activation='relu'))           # 3072 -> latent

            # Setting Decoder architecture
            self.decoder.add(Dense(32 * 32 * 3, activation='relu'))                     # latent -> 3072
    ...
    def train_autoencoder(self, epochs=3):
        ...
        plt.savefig('cp_fluid/losses/loss' + str(self.id_topology) + '_latent' + str(self.latent_dimension) + '.png')
        if self.plot:
            plt.show()
        plt.clf()
